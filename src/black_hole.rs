...
fn move_black_hole(
    mut black_hole_query: Query<(&mut Transform, &BlackHole)>,
    time: Res<Time>,
) {
    for (mut transform, black_hole) in black_hole_query.iter_mut() {
        transform.translation = transform.translation.lerp(black_hole.target_pos, 50./transform.translation.distance(black_hole.target_pos) * time.delta_seconds());
    }
}
    ...
    for (hole_tf, hole) in black_hole_query.iter_mut() {
        for (mut force, mob_tf) in mob_query.iter_mut() {
            if hole_tf.translation.distance(mob_tf.translation) <= 64. {
                let direction = (hole_tf.translation - mob_tf.translation).truncate().normalize_or_zero();
